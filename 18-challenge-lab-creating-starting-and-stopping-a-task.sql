...
-- 18.1.1  If a schema called SQUIRREL_DB.TASKS_SCHEMA does not exist, create it
--         now and set your context.
...
CREATE SCHEMA IF NOT EXISTS SQUIRREL_DB.TASKS_SCHEMA;
...
AS    
    INSERT OVERWRITE INTO SQUIRREL_DB.TASKS_SCHEMA.AVG_SHIPPING_IN_DAYS (yr, mon, avg_shipping_days)
    SELECT
          YEAR(F.O_ORDERDATE) AS YR,
          MONTH(F.O_ORDERDATE) AS MON,
          AVG (DAYS_TO_SHIP)::DECIMAL(18,2) AS AVG_DAYS_TO_SHIP
