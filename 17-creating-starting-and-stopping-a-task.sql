...
CREATE OR REPLACE SCHEMA SQUIRREL_DB.TASKS_SCHEMA;
USE SCHEMA SQUIRREL_DB.TASKS_SCHEMA;
...
WHERE database_name = 'SQUIRREL_DB';
...
CREATE WAREHOUSE IF NOT EXISTS SQUIRREL_wh INITIALLY_SUSPENDED=true;
USE WAREHOUSE SQUIRREL_wh;


-- 17.2.2  Create your new schemas.

CREATE OR REPLACE SCHEMA SQUIRREL_db.airline_tasks;
CREATE OR REPLACE SCHEMA SQUIRREL_db.airline_data;


-- 17.2.3  Create a table for staging new data.
--         Create this staging table in your airline_data schema.

CREATE OR REPLACE TRANSIENT TABLE SQUIRREL_db.airline_data.staging (
    country VARCHAR,
    region VARCHAR,
    name VARCHAR,
    code1 VARCHAR(5),
...
CREATE OR REPLACE TABLE SQUIRREL_db.airline_data.us_airlines (
   country VARCHAR,
   region VARCHAR,
   name VARCHAR, 
   alliance VARCHAR);
           
CREATE OR REPLACE TABLE SQUIRREL_db.airline_data.dive_destinations (
   country VARCHAR,
   name VARCHAR);
...
AS
   COPY INTO 
      SQUIRREL_db.airline_data.staging
   FROM 
...
AS 
   MERGE INTO SQUIRREL_db.airline_data.us_airlines u
   USING SQUIRREL_db.airline_data.staging s
   ON u.name = s.name
...
CREATE TASK load_dive_destinations
   AFTER load_staging_table
AS
   INSERT INTO SQUIRREL_db.airline_data.dive_destinations (country, name)
   SELECT country, name
   FROM SQUIRREL_db.airline_data.staging
...
AS
   TRUNCATE TABLE SQUIRREL_db.airline_data.staging;
...
FROM 
   ...
   (task_name => 'SQUIRREL_db.airline_tasks.load_staging_table'));
...
FROM 
   TABLE(information_schema.task_dependents
   (task_name => 'SQUIRREL_db.airline_tasks.load_staging_table', RECURSIVE => FALSE));
...
SELECT SYSTEM$TASK_DEPENDENTS_ENABLE('SQUIRREL_db.airline_tasks.load_staging_table');
...
FROM 
   TABLE(information_schema.task_history())
WHERE database_name = 'SQUIRREL_DB' AND schema_name = 'AIRLINE_TASKS';
...
SELECT * FROM SQUIRREL_db.airline_data.staging;

SELECT * FROM SQUIRREL_db.airline_data.us_airlines
ORDER BY name;

SELECT * FROM SQUIRREL_db.airline_data.dive_destinations
ORDER BY name;
...
TRUNCATE TABLE SQUIRREL_db.airline_data.dive_destinations;
...
CREATE OR REPLACE TASK load_dive_destinations
   AFTER load_staging_table
AS
   MERGE INTO SQUIRREL_db.airline_data.dive_destinations d
   USING SQUIRREL_db.airline_data.staging s
   ON d.name = s.name
   WHEN NOT MATCHED 
...
DROP TABLE SQUIRREL_db.airline_data.us_airlines;
...
DROP SCHEMA SQUIRREL_db.airline_tasks;
DROP SCHEMA SQUIRREL_db.airline_data;
