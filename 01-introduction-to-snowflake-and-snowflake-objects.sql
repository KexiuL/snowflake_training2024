...
-- 1.3.7   Open the documentation from the user menu.
...
-- 1.3.8   Click the Reference link at the top of the page and then click the
--         SQL Command Reference link from the list returned.

-- 1.3.9   Browse the SQL command reference.
...
-- 1.3.10  Click the Snowflake logo in the upper left corner of the web page to
--         return to the main page.

-- 1.3.11  Search for virtual warehouse in the search bar.
...
-- 1.3.12  Return to the Snowsight UI.
...
-- 1.4.9   Click the Snowflake logo in the upper left corner to return to the
--         home screen.
...
CREATE OR REPLACE WAREHOUSE SQUIRREL_wh WITH
...
--         see TRAINING_ROLE as your role and SQUIRREL_wh as your virtual
--         warehouse.
...
--         TRAINING_ROLE, selecting your virtual warehouse, SQUIRREL_wh, in the
...
-- 1.6.1   Create a database called SQUIRREL_db.

CREATE OR REPLACE DATABASE SQUIRREL_db;


-- 1.6.2   Create a schema called my_schema in the database you just created.

CREATE OR REPLACE SCHEMA SQUIRREL_db.my_schema;

USE SCHEMA SQUIRREL_db.my_schema;
...
ALTER USER SQUIRREL SET
   DEFAULT_ROLE = training_role
   DEFAULT_WAREHOUSE = SQUIRREL_wh
   DEFAULT_NAMESPACE = SQUIRREL_db.my_schema;
