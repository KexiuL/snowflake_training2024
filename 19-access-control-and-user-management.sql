...
USE SQUIRREL_DB.SQUIRREL_SCHEMA;
...
-- 19.1.3  Create a database called SQUIRREL_CLASSIFIED_DB.

CREATE DATABASE SQUIRREL_CLASSIFIED_DB;


-- 19.1.4  Create a table.
--         Using the role TRAINING_ROLE, create a table named SUPER_SECRET_TBL
--         inside the SQUIRREL_CLASSIFIED_DB.PUBLIC schema.

USE SCHEMA SQUIRREL_CLASSIFIED_DB.PUBLIC;
...
GRANT USAGE ON WAREHOUSE SQUIRREL_WH TO ROLE PUBLIC;
...
USE ROLE PUBLIC;
SELECT * FROM SQUIRREL_CLASSIFIED_DB.PUBLIC.SUPER_SECRET_TBL;
...
-- 19.1.8  Grant role PUBLIC usage on all schemas in SQUIRREL_CLASSIFIED_DB.

USE ROLE TRAINING_ROLE;

GRANT USAGE ON DATABASE SQUIRREL_CLASSIFIED_DB TO ROLE PUBLIC;
GRANT USAGE ON SCHEMA SQUIRREL_CLASSIFIED_DB.PUBLIC TO ROLE PUBLIC;
...
-- 19.1.9  Drop the database SQUIRREL_CLASSIFIED_DB.

USE ROLE TRAINING_ROLE;
DROP DATABASE SQUIRREL_CLASSIFIED_DB;
...
-- 19.2.3  Create a database called SQUIRREL_ROLETEST_DB.

CREATE DATABASE SQUIRREL_ROLETEST_DB;
USE SQUIRREL_ROLETEST_DB.PUBLIC;
...
USE ROLE PUBLIC;
SELECT * FROM SQUIRREL_ROLETEST_DB.PUBLIC.ROLE_TBL;
...
ALTER TABLE SQUIRREL_ROLETEST_DB.PUBLIC.ROLE_TBL ADD COLUMN name STRING(20);
SELECT * FROM SQUIRREL_ROLETEST_DB.PUBLIC.ROLE_TBL;

--         This should work since the roles granted to your user include
--         TRAINING_ROLE, the owner of the table.

-- 19.2.7  Try creating a new table in the SQUIRREL_ROLETEST_DB.

CREATE TABLE SQUIRREL_ROLETEST_DB.PUBLIC.NOROLE_TBL (name STRING(20));
...
USE ROLE TRAINING_ROLE;
GRANT SELECT ON SQUIRREL_ROLETEST_DB.PUBLIC.ROLE_TBL TO ROLE PUBLIC;
...
-- 19.2.11 Grant role PUBLIC usage on all schemas in SQUIRREL_ROLETEST_DB.

USE ROLE TRAINING_ROLE;
GRANT USAGE ON DATABASE SQUIRREL_ROLETEST_DB TO ROLE PUBLIC;
GRANT USAGE ON SCHEMA SQUIRREL_ROLETEST_DB.PUBLIC TO ROLE PUBLIC;
...
DROP DATABASE SQUIRREL_ROLETEST_DB;
